...
void stringAppend(char(&dest)[255], const char* src) {
...
void GetTime(char(&sendBuff)[255]) {
	...
	stringAppend(sendBuff, ctime(&timer));
}
...
void GetTimeWithoutDate(char(&sendBuff)[255]) {
	...
	strftime(sendBuff, 26, "%H:%M:%S", tm_info);
	puts(sendBuff);
}

void GetTimeSinceEpoch(char(&sendBuff)[255]) {
	time_t timer = time(NULL);
	snprintf(sendBuff, 16, "%lu", timer);
}
...
void GetTimeWithoutDateOrSeconds(char(&sendBuff)[255]) {
	...
	strftime(sendBuff, 26, "%H:%M", tm_info);
	puts(sendBuff);
}
...
void GetYear(char(&sendBuff)[255]) {
	...
	strftime(sendBuff, 26, "%Y", tm_info);
	puts(sendBuff);
}
...
void GetMonthAndDay(char(&sendBuff)[255]) {
	...
	strftime(sendBuff, 26, "%m/%d", tm_info);
	puts(sendBuff);
}

void GetSecondsSinceBeginingOfMonth(char(&sendBuff)[255]) {
	time_t begginingOfMonth, currentTime, result;
	struct tm* tmBOM, *tmResult;
	time(&currentTime);
	tmBOM = localtime(&currentTime);
	tmBOM->tm_hour = 0;
	tmBOM->tm_min = 0;
	tmBOM->tm_sec = 0;
	tmBOM->tm_mday = 1;
	begginingOfMonth = mktime(tmBOM);
	result = currentTime - begginingOfMonth;
	tmResult = localtime(&result);

	snprintf(sendBuff, 16, "%lu", tmResult);
}
...
void GetWeekOfYear(char(&sendBuff)[255]) {
	...
	struct tm* tmWOY;
	time(&currentTime);
	tmWOY = localtime(&currentTime);
	weeks = tmWOY->tm_yday / 7;

	snprintf(sendBuff, 32, "%d", weeks);
}
...
void GetDaylightSavings(char(&sendBuff)[255]) {
	...
	snprintf(sendBuff, 32, "%d", tmNow->tm_isdst);
}

void addHours(tm* tmNow, int hours) {
	if (tmNow->tm_hour + hours >= 24) {
		tmNow->tm_hour = tmNow->tm_hour + hours - 24;
	}
	else if (tmNow->tm_hour + hours < 0) {
		tmNow->tm_hour = 24 + (tmNow->tm_hour + hours);
	}
	else {
		tmNow->tm_hour += hours;
	}
...
void GetTimeWithoutDateInCity(char(&sendBuff)[255]) {
	char temp[255];
	time_t currentTime;
	struct tm* tmNow;
	...
	sendBuff[0] = '\0';
	time(&currentTime);
	tmNow = gmtime(&currentTime);

	//UTC/GMT 0
	strftime(temp, 26, "%H:%M:%S", tmNow);
	stringAppend(sendBuff, "UTC TIME: ");
	stringAppend(sendBuff, temp);
	stringAppend(sendBuff, "\n");
	//Japan +9
	addHours(tmNow, 9);
	strftime(temp, 26, "%H:%M:%S", tmNow);
	stringAppend(sendBuff, "TOKYO TIME: ");
	stringAppend(sendBuff, temp);
	stringAppend(sendBuff, "\n");
	addHours(tmNow, -9);
	//Australia +10
	addHours(tmNow, 10);
	strftime(temp, 26, "%H:%M:%S", tmNow);
	stringAppend(sendBuff, "Australia TIME: ");
	stringAppend(sendBuff, temp);
	stringAppend(sendBuff, "\n");
	addHours(tmNow, -10);
	//San Francisco -7
	addHours(tmNow, -7);
	strftime(temp, 26, "%H:%M:%S", tmNow);
	stringAppend(sendBuff, "San Francisco TIME: ");
	stringAppend(sendBuff, temp);
	stringAppend(sendBuff, "\n");
	addHours(tmNow, 7);
	//Portogal +1
	addHours(tmNow, 1);
	strftime(temp, 26, "%H:%M:%S", tmNow);
	stringAppend(sendBuff, "Portogal TIME: ");
	stringAppend(sendBuff, temp);
	stringAppend(sendBuff, "\n");
	addHours(tmNow, -1);
}

void actions(char recvBuff[255], char(&sendBuff)[255]) {
	...
	} else if (!strcmp(recvBuff, "10")) {
		GetWeekOfYear(sendBuff);
	} else if (!strcmp(recvBuff, "11")) {
		GetDaylightSavings(sendBuff);
	} else if (!strcmp(recvBuff, "12")) {
		GetTimeWithoutDateInCity(sendBuff);
	} else if (!strcmp(recvBuff, "13")) {
